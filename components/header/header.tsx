...
export default function HeaderComponent() {
    ...
    return (
        <>
            ...
            <header className="fixed top-[40px] md:top-[48px] z-20 w-full px-4 py-2 bg-black/80 flex justify-between items-center">
                ...
                <nav className="hidden md:flex items-center space-x-6">
                    ...
                    <div className="relative group">
                        <Link href="/activities" className="text-white flex items-center gap-1 hover:text-yellow-300 transition">
                            ACTIVITIES <ChevronDown size={14} />
                        </Link>
                        <div className="absolute hidden group-hover:block bg-white text-black mt-2 rounded-xl shadow-2xl z-10 w-64 border border-gray-200 max-h-[400px] overflow-y-auto">
            ...
            {/* Mobile Menu */}
            {mobileMenuOpen && (
                <div className="fixed inset-0 bg-black/90 z-10 flex flex-col items-start justify-start pt-24 pl-6 pr-4 space-y-4 md:hidden overflow-y-auto max-h-screen">
                    {navLinks.slice(0, 2).map(({ href, label }) => (
                        <Link key={href} href={href} className="text-white hover:text-gray-200" onClick={() => setMobileMenuOpen(false)}>{label}</Link>
                    ))}

                    {/* Mobile Collapsible Activities */}
                    <button
